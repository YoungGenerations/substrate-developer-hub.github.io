...
The first step towards building a Substrate runtime module is to define what other PALETTE modules we could use in our module. There are plenty of PALETTE modules that ship with the Substrate codebase and we recommend using them, when needed. To use any of the existing modules, we need to import and specify them in the module trait of our custom module.

For example, in this runtime we need the capability to calculate and compare timestamps for the TCR parameters - apply stage length and commit stage length. We will use the [`timestamp` PALETTE module](https://github.com/paritytech/substrate/tree/master/palette/timestamp) to achieve this functionality.
