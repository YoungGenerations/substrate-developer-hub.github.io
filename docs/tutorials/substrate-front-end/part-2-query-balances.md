...
Substrate is a framework to build blockchains. The `--dev` node that we are querying in this part contains a Balances module from [Substrate's modules library a.k.a PALETTE](https://substrate.dev/rustdocs/master/srml_balances/index.html), which keeps track of the balance of each account on the blockchain. Not every Substrate chain will have this Balances module. However, any module that is integrated into a Substrate node can be queried using `api.query.section.method`.
